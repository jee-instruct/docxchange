      /*
       * Use Angular from Here for the docxchange Project..
       * @Vamsee Chowdhary
       * 
       */

var myApp = angular.module('docxchange', ['ui.bootstrap','nvd3']);
...
myApp.controller('autoSuggestCtrl', function($scope,$http,limitToFilter, dataFactory,$location) { 
	 $scope.query = '';
	 $scope.itemsPerPage = 10;
	  $scope.currentPage = 0;
	  $scope.host = $location.path();
	 $scope.getSuggestions = function(query) {
			 dataFactory.get($location.path()+"/docxchange/secure/api/suggestions?query="+query).then(function(response) {
				 $scope.suggestions = response.data;
			 });
		  };
		  
		  $scope.onSelect = function ($item, $model, $label) { 
			  dataFactory.get($location.path()+"/docxchange/secure/api/search?query="+$scope.query).then(function(response) {
					 $scope.searchResults = response.data.results;
					 $scope.noOfHits = response.data.hits;
					 $scope.searchTime = response.data.searchTime;
				 }); 
			  };
			  ...
			  $scope.onKeyup = function() {
			    ...
			    $scope.range = function() {			    	
			        ...
			        var rangeSize = 10;
			        var size = Math.ceil($scope.searchResults.length/$scope.itemsPerPage);
...
myApp.controller('dashboardCtrl', function($scope,dataFactory) {
	/* Chart options */
    $scope.donutOptions = {
            chart: {
                type: 'pieChart',
                ...
                showLabels: true,
                // delete this if you want the complete donut circle
                pie: {
    ...
    $scope.multiBarHorizontalChartOptions = {
            chart: {
    ...
    $scope.multiBarHorizontalChartData = [
                   {
                       "key": "Series1",
                       "color": "#d62728",
                       "values": [
